...
def main():
    ...
    model, instance_model, bag_level_uncertainty_model = build_model(data_dims=[28,28])

    model.summary()
    # model.fit(train_data, batch_size=bag_size, epochs=1, steps_per_epoch=1)
    for e in range(epochs):
        model.fit(train_data, batch_size=bag_size, epochs=1)
    ...
    visualize_attention(model, instance_model, test_data_instances, save_dir, quick_eval=False)
