...
def visualize_attention(bag_model, instance_model, test_gen_instances, save_dir, quick_eval=True):
    n_visualize = 100
    ...
    for i in range(n):
        ...
        if i < n_visualize:
            _plot_images(x_bag, bag_pred, mean, std, os.path.join(save_dir + str(i)) + '.jpg')
        else:
            if quick_eval:
                break
